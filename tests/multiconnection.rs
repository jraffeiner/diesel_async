...
async fn establish_connection() -> InferConnection {
    ...
    InferConnection::establish(&database_url).await.unwrap()
}

async fn make_test_table(conn: &mut InferConnection) {
    match conn {
        #[cfg(feature = "postgres")]
...
async fn type_checks() {
    ...
    let mut conn = establish_connection().await;
    make_test_table(&mut conn).await;
    conn.begin_test_transaction().await.unwrap();
    let small_int = 1_i16;
    let integer = 2_i32;
    let big_int = 3_i64;
